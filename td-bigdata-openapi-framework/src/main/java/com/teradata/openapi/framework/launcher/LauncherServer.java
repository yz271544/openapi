 ...
 *
 * The launcher server is used when Openapi apps are launched as separate processes than the calling
 * app. It looks more or less like the following:
 *
 *         -----------------------                       -----------------------
 *         |      User App       |     openapi-submit      |      Openapi App      |
 *         |                     |  -------------------> |                     |
 *         |         ------------|                       |-------------        |
 *         |         |           |        hello          |            |        |
 *         |         | L. Server |<----------------------| L. Backend |        |
 ...
 * ({@link OpenapiLauncher#CHILD_CONNECTION_TIMEOUT}), at which point the server will throw away
...
class LauncherServer implements Closeable {
  ...
  private long getConnectionTimeout() {
    String value = OpenapiLauncher.launcherConfig.get(OpenapiLauncher.CHILD_CONNECTION_TIMEOUT);
    ...
    protected void handle(Message msg) throws IOException {
      try {
        if (msg instanceof Hello) {
          ...
          if (handle != null) {
            handle.setState(OpenapiAppHandle.State.CONNECTED);
