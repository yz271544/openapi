...
/**
  ...
  */
class FtchSyncByJdbcTd extends SyncDbTaskPlugin[SyncStep] with OpenApiLogging {
  ...
  override def driverTableSql(step: SyncStep): String = {
    ...
    s"select $titleContext from %s where 1=1 %s qualify sum(1) over(rows unbounded preceding) between %s and %s".format(sourceTableName, SqlUtil.sourceCondition(step.source.source_id, step.source.source_type_code, step.reqArgs).toString, s"($page_num - 1) * $page_size + 1", s"($page_num * $page_size)")
  }
