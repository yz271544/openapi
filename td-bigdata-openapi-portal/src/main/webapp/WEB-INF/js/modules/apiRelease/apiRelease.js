	...
	var getFormJson = function(form_id) {
		...
		$.each(a, function () {
			if (o[this.name] !== undefined) {
				...
				if(this.value){
					o[this.name] = this.value + ','+ o[this.name];
				}
			} else {
			...
			$.ajax({   
			     ...
			     success: function(data) {
			    	 ...
			    	 $.each(data.items, function(i, item) {
			        	 ...
			        	 if(item.examStat == 2){
			        		 sb.append('<td> <div class="td-label text-white text-center popovers label-success" data-container="body" data-trigger="hover" data-placement="top" data-content="'+item.auditAdvc+'" data-original-title="审批意见">'+item.examStatDesc+'</div></td>');
			        	 }else if(item.examStat == 3){
			        		 sb.append('<td> <div class="td-label text-white text-center popovers label-danger" data-container="body" data-trigger="hover" data-placement="top" data-content="'+item.auditAdvc+'" data-original-title="审批意见">'+item.examStatDesc+'</div></td>');
			        	 }else{
			        	 ...
			        	 sb.append('>');
			        	 sb.append(	 '<a href="javascript:;" class="btn green btn-sm btn-outline sbold uppercase btn_cloneApiItem toolTip" title="克隆">');
			        	 sb.append(		'<i class="fa fa-copy"></i></a>');
			        	 ...
			        	 sb.append(	 '<a href="javascript:;" class="btn green btn-sm btn-outline sbold uppercase btn_updateApiItem toolTip'+hide_btn_updateApiItem+'" data-toggle="modal" data-target="#modal_apiItem" action="updateApiItem" title="修改">');
			        	 sb.append(		'<i class="fa fa-edit"></i></a>');
			        	 sb.append(	 '<a href="javascript:;" class="btn green btn-sm btn-outline sbold uppercase btn_releaseApiItem toolTip'+hide_btn_releaseApiItem+'" title="发布">');
			        	 sb.append(		'<i class="fa fa-plus"></i></a>');
			        	 sb.append(	 '<a href="javascript:;" class="btn green btn-sm btn-outline sbold uppercase btn_deleteApiItem toolTip" title="删除">');
				...
				if (!showError(data)) {
					...
					$.each(apiArg, function(index, apiInfo){
						...
						$.each(fieldList, function(index, field){
							...
							var mustTypeDesc = field.mustType == 1 ? field.mustTypeDesc : '未选';
							var mustOneGrpId = field.mustOneGrpId == null ? '暂无' : field.mustOneGrpId;
	...
	return {
        init: function () {
            ...
            $('body').on('click', '#btn_configApiArg', function(){
				...
				var apiName = $('#txt_apiName_tmp').val();
				var sb = new StringBuffer();
				var $apiVisitMethd = $('input[name="apiVisitMethd"]:checked');
				$.each($apiVisitMethd, function(i, item){
					var surffix = (i+1) != $apiVisitMethd.length ? ',' : '';
					sb.append($(item).val() + surffix);
				});
            	window.open($.basePath+'/apiRelease/configApiArg.htm?apiId='+apiId+'&apiVersion='+apiVersion+'&apiName='+apiName+'&apiVisitMethd='+sb.toStr());
            });
        }
    };
}();
