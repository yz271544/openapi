var View = function() {
	var hidePortlet1 = function(){
		$('#portlet_1').hide();
	}
	
	var handlePortletBody1 = function() {
		App.blockUI({
			target : '#portlet_body_1',
			message : '加载中...'
		});
		
		var $registerNum = $('#register_num');
		var $authNum = $('#auth_num');
		var $approveNum = $('#approve_num');

		var data_url = $.basePath + '/home/getExamCount.json';
		var data_param = {};
		$.ajax({
			...
			success : function(data) {
				var item = data[0];
				var registerNum = item.registerNum;
				var authNum = item.authNum;
				var approveNum = item.approveNum;
				$registerNum.text(registerNum);
				$authNum.text(authNum);
				$approveNum.text(approveNum);
				App.unblockUI('#portlet_body_1');
			},
	...
	var handlePortletBody2 = function(chartId, itemId, chartUnit) {
		App.blockUI({
		...
		var data_url = $.basePath + '/home/getApiChart.json';
		var data_param = {
			itemId : itemId
		};
		$.ajax({
	...
	var handlePortletBody3 = function(){
		handlePortletContent3_1();
		handlePortletContent3_2();
	}
	var handlePortletContent3_1 = function() {
		App.blockUI({
			target : '#portlet_content_3_1',
			message : '加载中...'
		});

		var $totalNum = $('#total_num');
		var $totalProgress = $('#total_progress');
		var $totalStatus = $('#total_status');

		var $activeNum = $('#active_num');
		var $activeProgress = $('#active_progress');
		var $activeStatus = $('#active_status');

		var $unactiveNum = $('#unactive_num');
		var $unactiveProgress = $('#unactive_progress');
		var $unactiveStatus = $('#unactive_status');

		var $errorNum = $('#error_num');
		var $errorProgress = $('#error_progress');
		var $errorStatus = $('#error_status');

		var data_url = $.basePath + '/home/getApiTotal.json';
		var data_param = {viewType : $('#viewType').val()};
		$.ajax({
			url : data_url,  
			type : "POST",
			data : data_param,
			...
			success : function(data) {
				...
				var activeNum = item.activeNum;
				var unactiveNum = item.unactiveNum;
				var errorNum = item.errorNum;
				var activePercent = (activeNum / totalNum * 100)
						.toFixed(0)
						+ '%';
				var unactivePercent = (unactiveNum / totalNum * 100)
						.toFixed(0)
						+ '%';
				var errorPercent = (errorNum / totalNum * 100)
						.toFixed(0)
						+ '%';

				$totalNum.attr('data-value', totalNum).counterUp();
				$totalProgress.css('width', '100%');
				$totalStatus.text('100%');

				$activeNum.attr('data-value', activeNum).counterUp();
				$activeProgress.css('width', activePercent);
				$activeStatus.text(activePercent);

				$unactiveNum.attr('data-value', unactiveNum).counterUp();
				$unactiveProgress.css('width', unactivePercent);
				$unactiveStatus.text(unactivePercent);

				$errorNum.attr('data-value', errorNum).counterUp();
				$errorProgress.css('width', errorPercent);
				$errorStatus.text(errorPercent);

				App.unblockUI('#portlet_content_3_1');
			},
			error : function(data, e) {
				bootbox.alert("系统错误,请稍候再试！");
			}
		...
		function initTable(pageIndex) {
			...
			$.ajax({
				...
				success : function(data) {
					...
					App.unblockUI('#portlet_content_3_2');
				},
				error : function(data, e) {
					bootbox.alert("系统错误,请稍候再试！");
				}
		...
		init : function() {
			...
			$('#nav_tabs_chart a[data-toggle="tab"]').on('shown.bs.tab',
					function(e) {
						...
						var chartUnit = $(e.target).attr('chartUnit');
						handlePortletBody2(chartId, itemId, chartUnit);
					});
			...
			$(document).on('click', '.more', function(){
				...
				window.location = $.basePath + moduleUrl;
			});
