...
import com.teradata.bigdata.openapi.rewrite.GBKTextOutputFormat
...
object FormatFunction {
	...
	def main(args: Array[String]) {
		...
		val inPath:String = otherArgs{0}
		val inFieldDelimiter:String = FormatFunctionUtil.checkDelimiter(otherArgs{1})
		val outPath:String = otherArgs{2}
		val outCode:String = otherArgs{3}.toUpperCase
		val outFieldDelimiter:String = FormatFunctionUtil.checkDelimiter(otherArgs{4})
		val formatType:String = otherArgs{5}.toUpperCase
		val titleString:String = otherArgs{6}.toUpperCase
...
/*		println(System.currentTimeMillis() + " : " + System.currentTimeMillis() + "inPath = |" + inPath + "|")
		...
		try {
			Encode.withName(outCode)
			Format.withName(formatType)
		} catch {
			case e:NoSuchElementException => println(System.currentTimeMillis() + " : " + e.getMessage); throw e
		}
		...
		DefaultStringifier.store(conf, new Text(outCode) , "outCode")
		DefaultStringifier.store(conf, new Text(outFieldDelimiter) , "outFieldDelimiter")
		DefaultStringifier.store(conf, new Text(formatType) , "formatType")
