...
public class NotExistErrorResponse extends ErrorResponse {
	...
	/**
	 * 对象不存在的错误对象。当根据<code>queryFieldName</code>查询<code>objectName</code>
	 * 时，查不到记录，则返回该错误对象。
	 * 
	...
	public NotExistErrorResponse(String objectName, String queryFieldName, Object queryFieldValue, Locale locale) {
		...
		SubError subError = SubErrors.getSubError(subErrorCode, SubErrorType.ISV_NOT_EXIST.value(), locale, queryFieldName,
		                                          queryFieldValue, objectName);
