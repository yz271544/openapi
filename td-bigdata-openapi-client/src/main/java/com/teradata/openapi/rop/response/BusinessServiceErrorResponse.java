...
public class BusinessServiceErrorResponse extends ErrorResponse {
	 ...
	 * serviceName为file.upload，error_code为INVLIAD_USERNAME_OR_PASSWORD
	 * ，则错误码会被格式化为： isv.file-upload-service-error:INVLIAD_USERNAME_OR_PASSWORD
	 * 
	 * @param context 请求上下文
	...
	public BusinessServiceErrorResponse(RopRequestContext context, String errorCode, Object... params) {
		MainError mainError = MainErrors.getError(MainErrorType.BUSINESS_LOGIC_ERROR, context.getLocale(), context.getMethod(),
		                                          context.getVersion());
	...
	/**
	 * 服务发生错误的错误响应，错误码的格式为：isv.***-service-error:###,假设
	 * serviceName为book.upload，error_code为INVLIAD_USERNAME_OR_PASSWORD
	 * ，则错误码会被格式化为： isv.book-upload-service-error:INVLIAD_USERNAME_OR_PASSWORD
	 * 
