...
import com.teradata.openapi.rop.RopRequestParseException;
import com.teradata.openapi.rop.impl.SimpleRopRequestContext;
	...
	private JAXBContext getJaxbContext(Class clazz) {
		...
		if (jaxbContext == null) {
			try {
				...
				throw new HttpMessageConversionException("Could not instantiate JAXBContext for class [" + clazz + "]: " + ex.getMessage(),
				        ex);
			}
		}
		return jaxbContext;
