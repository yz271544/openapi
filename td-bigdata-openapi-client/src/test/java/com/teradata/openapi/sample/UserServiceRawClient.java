...
public class UserServiceRawClient extends AbstractJUnit4SpringContextTests {
	...
	public void createSession() {
		...
		// assertTrue(response.indexOf("<logonResponse sessionId=\"mockSessionId1\"/>")
		// > -1);
	}
	...
	public void testQueryUserByUserId() {
		...
		// 使用GET获取：正确返回
		String response = restTemplate.getForObject(SERVER_URL + "?method={method}&appKey={appKey}&v={v}&sessionId={sessionId}&locale={locale}"
		        + "&userId={userId}&sign={sign}", String.class, form);
	...
	public void testServiceXmlRequestAttr() {
		...
		// address会正确绑定
		form.add("address", "<address zoneCode=\"0001\" doorCode=\"002\">\n" + "  <streets>\n" + "    <street no=\"001\" name=\"street1\"/>\n"
		...
		String response = restTemplate.postForObject(SERVER_URL, form, String.class);
		// System.out.println("response:\n" + response);
		// assertTrue(response.indexOf("<createUserResponse createTime=\"20120101010101\" userId=\"1\">")
		// > -1);
	}
	...
	public void testServiceJsonRequestAttr() {
		...
		// address会正确绑定
		form.add("address", "{\"zoneCode\":\"0001\",\n" + " \"doorCode\":\"002\",\n" + " \"codes\":[\"aa\",\"bb\",\"cc\"],\n"
		        + " \"streets\":[{\"no\":\"001\",\"name\":\"street1\"},\n" + "            {\"no\":\"002\",\"name\":\"street2\"}]}");
		// form.add("favorites", "\"aa\",\"bb\",\"cc\"");

		form.add("addresses", "[{\"zoneCode\":\"0001\",\n" + " \"doorCode\":\"002\",\n" + " \"codes\":[\"aa\",\"bb\",\"cc\"],\n"
		        + " \"streets\":[{\"no\":\"001\",\"name\":\"street1\"},\n" + "            {\"no\":\"002\",\"name\":\"street2\"}]}]");
		form.add("addresses", "[{\"zoneCode\":\"0001\"},{\"zoneCode\":\"0001\"}]");
		form.add("favorites", "\"aa\",\"bb\",\"cc\"");
	...
	/**
	 * 由于{@link com.rop.sample.request.CreateUserRequest#password}标注了 {@link com.rop.annotation.IgnoreSign},所以Rop 会忽略对password请求参数进行签名验证。
	 */
	@Test
	...
	public void testIngoreSign() {
		...
		form.add("sessionId", "mockSessionId1");
		form.add("salary", "2,500.00");
		// String sign = RopUtils.sign(form.toSingleValueMap(),
	...
	/**
	 * 请求参数违反合法性校验限制时，返回相应的错误报文 服务端限制salary的格式必须是“#,###.##”，且必须在1000.00和100000.00之间， 参见
	 * {@link com.rop.sample.request.CreateUserRequest#salary}
	 */
	@Test
	...
	/**
	 * 在类路径的rop.appSecret.properties中我们定义了两个合法的appKey,分别是： 00001=abcdeabcdeabcdeabcdeabcde 00002=abcdeabcdeabcdeabcdeaaaaa
	...
	public void testInvalidSysParams() {
		...
		// 使用GET获取：正确返回
		String response = restTemplate.getForObject(SERVER_URL
		        + "?appKey={appKey}&locale={locale}&method={method}&salary={salary}&sessionId={sessionId}"
		        + "&userName={userName}&v={v}&sign={sign}", String.class, form);
		System.out.println("response:" + response);
	}
	...
	public void testI18nErrorMessage() {
		...
		// 使用GET获取：正确返回
		response = restTemplate.getForObject(SERVER_URL
		        + "?appKey={appKey}&locale={locale}&method={method}&salary={salary}&sessionId={sessionId}"
		        + "&userName={userName}&v={v}&sign={sign}", String.class, form);
		System.out.println("response:" + response);
	}
	 ...
	 * {@link UserService#addUser(com.rop.sample.request.CreateUserRequest)} 的权限，而sessionId为mockSessionId2时则没有这个权限。 将返回会话错误的响应报文。
	 */
	@Test
	...
	/**
	 * 在{@link SampleSessionChecker}中，我们模拟了一个合法的sessionId,即 mockSessionId1，使用其它的sessionId， 将返回会话错误的响应报文。
	 */
	@Test
	...
	/**
	 * “jhon”是 {@link UserService#addUser(com.rop.sample.request.CreateUserRequest)} 服务方法预留的用户名， 验证其会返回正确的业务错误码。
	 */
	@Test
	...
	public void testGetUserOf9999() {
		...
		// 使用GET获取：正确返回
		String response = restTemplate.getForObject(SERVER_URL + "?method={method}&appKey={appKey}&v={v}&sessionId={sessionId}&locale={locale}"
		        + "&userId={userId}&salary={salary}&sign={sign}", String.class, form);
		System.out.println("response:" + response);
	...
	public void testStreamOutput() {
		...
		// 使用GET获取：正确返回
		String response = restTemplate.getForObject(SERVER_URL + "?method={method}&appKey={appKey}&v={v}&sessionId={sessionId}&locale={locale}"
		        + "&messageFormat={messageFormat}&sign={sign}", String.class, form);
		// System.out.println("response:" + response);
